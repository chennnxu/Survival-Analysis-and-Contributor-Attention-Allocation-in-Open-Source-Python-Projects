...
threePalette <- c("#800080", "#0072B2", "#FF0000")  # Orange, Sky blue, Green
twoPalette <- c("#0072B2", "#FF0000")  # Sky blue, Orange
```
...
ggsave("km_all.pdf", plot = last_plot(), device = "pdf")
```
...
ggsave("km_host_type.pdf", plot = last_plot(), device = "pdf")
```
...
ggsave("km_multi_repo.pdf", plot = last_plot(), device = "pdf")
```
...
ggsave("km_author_count.pdf", plot = last_plot(), device = "pdf")
```
...
summary(fit6)
ggsurvplot(fit6, data = a_data, pval = TRUE, conf.int = TRUE, legend.labs = c("rev. per day <= 1", "rev. per day > 1"), xlab="Time (Months)", palette=twoPalette)
ggsave("km_rev_freq.pdf", plot = last_plot(), device = "pdf")
```
...
fit.coxph <- coxph(surv_object ~ host_type + multi_repo + high_author_count + high_rev_freq, data = a_data)

ggforest(fit.coxph, data = a_data, fontsize=0.9)
ggsave("cox.pdf", plot = last_plot(), device = "pdf")
```
